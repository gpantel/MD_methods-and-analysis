...
if not os.path.exists(os.getcwd()+'/absPsi_movie'):
    os.makedirs(os.getcwd()+'/absPsi_movie')
...
########################## Atom selection end ##########################
        ...
        psi = [np.linalg.norm(x) for x in psi_frame]
        Pxyz = all_coords
        Pxy = []
...
#        voronoi_plot_2d(vor_s)  #plot the lines on the tesselation
        for p in range(0, len(vor_points)):
            ...
            if not -1 in rgn and 0 <= L[0] < x_box and 0 <= L[1] < y_box :
                ...
                plt.fill(*zip(*polygon), color=cm.RdBu(L[2]))
        plt.ylabel(r'y ($\AA$)', fontsize=24)
        plt.xlabel(r'x ($\AA$)', fontsize=24)

        cax = ax.imshow(np.array([[1,1],[1,1]]), cmap=cm.RdBu, vmin=0, vmax=1)
        ...
        plt.tight_layout()
        if side == 'up':
            plt.savefig('absPsi_movie/'+'upper_img' + str('%05d'%frame) + '.png')
        if side == 'down':
            plt.savefig('absPsi_movie/'+'lower_img' + str('%05d'%frame) + '.png')
    return
